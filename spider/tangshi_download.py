...
def process_and_save_content(link):
    ...
    if response.status_code == 200:
        ...
        if title:
            ...
            # 使用 CSS 选择器提取文本内容
            content = soup.select_one(".main3 > .left > #sonsyuanwen > .cont > div:nth-of-type(2)").get_text(strip=True)

            # 保存到本地文件
            save_to_file(title_text, content)

        else:
            print(f"Failed to extract title from {link}")
