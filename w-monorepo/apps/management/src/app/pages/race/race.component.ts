...
import { WEAPON_CLASSIFIERS } from '../../shared/constants/weapon.constants';
...
export class RaceComponent implements OnInit {
  ...
  getAllWarhammerClassifiers(): void {
    this.http
      .get<any[]>('http://localhost:8080/warhammerClassifier/findAll')
      .subscribe(
        (classifiers) => {
  ...
  saveAllWarhammerClassifiers() {
    WEAPON_CLASSIFIERS.forEach((classifier: IWarhammerClassifier) => {
  ...
  saveWarhammerClassifier(classifier: {
    id: string;
    directory: string;
    nameCN: string;
    ...
    this.http
      .post(
        'http://localhost:8080/warhammerClassifier/saveWarhammerClassifier',
        classifier
      )
      .subscribe(
        (response) => {
