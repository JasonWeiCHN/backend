...
const scaleY = (value) => padding + (canvas.height - 2 * padding) * (1 - (value - yMin) / (yMax - yMin));
...
for (let i = 0; i < history.length - 1; i++) {
  ...
  // Draw vertical line if y-value changes
...
canvas.addEventListener('mousemove', (e) => {
  ...
  for (let i = 0; i < history.length - 1; i++) {
    ...
    if (mouseX >= x1 && mouseX <= x2 && Math.abs(mouseY - y1) < 5) {
      const date = new Date(history[i].x).toLocaleString();
      tooltip.style.left = `${e.clientX + 10}px`;
      tooltip.style.top = `${e.clientY + 10}px`;
      tooltip.style.display = 'block';
      tooltip.textContent = `时间: ${date}, 价格: ${history[i].f}`;
      return;
    }

    if (history[i].y !== history[i + 1].y && Math.abs(mouseX - x2) < 5 && mouseY >= Math.min(y1, y2) && mouseY <= Math.max(y1, y2)) {
      const date = new Date(history[i + 1].x).toLocaleString();
      tooltip.style.left = `${e.clientX + 10}px`;
      tooltip.style.top = `${e.clientY + 10}px`;
      tooltip.style.display = 'block';
      tooltip.textContent = `时间: ${date}, 价格: ${history[i + 1].f}`;
      return;
    }
  }
