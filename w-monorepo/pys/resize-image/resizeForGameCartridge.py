...
def generate_unique_id(length=6):
    """生成唯一 ID，例如 F_1qug5y"""
    return "F_" + ''.join(random.choices(string.ascii_letters + string.digits, k=length))
...
def extract_title_price(filename):
    ...
    price = int(price_match.group(1)) if price_match else 0  # 提取价格中的数字
        ...
        if os.path.isfile(input_path):
            try:
                ...
                image_data = {
                    ...
                    "detail": "",
                    "id": unique_id,
                    "imageUrl": f"assets/images/ps5/{unique_id}",
                    "tagIds": [],
                    "title": title,
                    "price": {
                        "discountPercent": 100,
